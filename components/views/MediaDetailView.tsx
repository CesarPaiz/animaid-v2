...
const MediaDetailView: React.FC<MediaDetailViewProps> = ({ media: initialMedia, onClose, onStartPlayback }) => {
    ...
    const renderableTotalUnits = useMemo(() => {
        ...
        // For releasing series where the final count isn't set, the next airing episode is the best source of truth.
        if (media.status === MediaStatus.RELEASING && media.nextAiringEpisode) {
            // The API gives the number of the *next* episode. The last available is one less.
    ...
    const fetchUserProgress = useCallback(async () => {
        ...
        const entry = await getMediaEntry(media.id);
        setMedia(prevMedia => ({
            ...prevMedia,
            userProgress: entry ? {
    ...
    const handleStatusUpdate = useCallback(async (newStatus: MediaListStatus) => {
        ...
        const previousUserProgress = media.userProgress;

        // Optimistic UI update
        setMedia(prevMedia => ({
            ...prevMedia,
            userProgress: {
                progress: prevMedia.userProgress?.progress || 0,
                score: prevMedia.userProgress?.score || 0,
                status: newStatus,
            }
        }));
        ...
        try {
            ...
            if (updatedEntry) {
                ...
                await fetchUserProgress(); // Fallback to refetch
            }
        } catch (error) {
            ...
            setMedia(prevMedia => ({
                ...prevMedia,
                userProgress: previousUserProgress
            }));
        }
    }, [user, media.id, media.userProgress, isManga, upsertMediaEntry, fetchUserProgress]);
    ...
    const renderStatusControls = () => {
        if (isLoadingProgress) return <div className="h-12"><Spinner/></div>;
